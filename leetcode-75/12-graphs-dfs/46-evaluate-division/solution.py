...
if __name__ == "__main__":
    ...
    # Edge cases
    assert_equal(
        sol.calcEquation([["a", "b"], ["b", "c"]], [2.0, 3.0], [["a", "f"]]),
        [-1.0],
        "query variable not in the graph at all",
    )

    assert_equal(
        sol.calcEquation([["a", "b"], ["b", "c"]], [2.0, 3.0], [["b", "b"]]),
        [1.0],
        "c==d and variable exists in the graph",
    )

    assert_equal(
        sol.calcEquation([["a", "b"], ["b", "c"]], [2.0, 3.0], [["z", "z"]]),
        [-1.0],
        "c==d but variable doesn't exist in the graph",
    )

    assert_equal(
        sol.calcEquation([["a", "b"], ["c", "d"]], [2.0, 3.0], [["a", "c"]]),
        [-1.0],
        "disconnected components, no path between existing variables",
    )

    assert_equal(
        [round(x, 5) for x in sol.calcEquation([["a", "b"], ["b", "c"]], [2.0, 3.0], [["c", "a"]])],
        [round(1 / 6, 5)],
